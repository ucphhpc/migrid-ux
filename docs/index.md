...
is _bound_ (see [binding](#binding)) to the on-screen HTML markup.

### Handling of state

The state is represented as a series of observed values which signal when they
are changed. Such values is commonly referred to as
[observables](#observable). These values are grouped into
[namespaces](#namespace) of related values, each of which can be individually
serialised on demand.
...
Knockout, but client side templating is omitted in favour of the use of
...
declarative definition of required values specified by a given application.
...
Running the above will have the effect of installing the necessary
dependencies for local development and provisioning a virtual environment to
allow the use of a fake backend intended to support rapid iteration during
development. As a final step a local web server and local backend will be
started, with the application then being accessible at
`http://localhost:8880`
...
The source code is licensed under GPLv2 or MIT as defined by the source in
question. These licenses can be found under the LICENSES directory.
