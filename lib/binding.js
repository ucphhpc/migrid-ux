...
# --- BEGIN_HEADER ---
# 
# binding.js - dual licensed source code file
...
const ATTR_ONMOUSEOVER = "data-bind-onmouseover";
...
const BIND_ONMOUSEOVER = "bindOnmouseover";
const BIND_ONMOUSEOVER_ARGUMENT = "bindOnmouseoverArgument";
...
class Binder {
    ...
    for (const attrName of [
      ...
      switch (attrName) {
        ...
        case ATTR_ONMOUSEOVER:
          this.bindOnmouseover(fromElement, namespace, context);
          break;
        case ATTR_SHOW:
  ...
  processPopup(popupEl, namespace) {
    this.processElement(popupEl, namespace, this.context);
    this.bindCssElement(popupEl, namespace);
    this.recordNamespaceElement(popupEl, namespace);
    this.elementFinder.excludeSubtree(popupEl);
  }
  ...
  bindOnmouseover(fromElement, namespace, context) {
    const inputElements = this.queryAttributesAll(fromElement, ATTR_ONMOUSEOVER);

    for (const inputEl of inputElements) {
      const method = context[inputEl.dataset[BIND_ONMOUSEOVER]];
      const arg = inputEl.dataset[BIND_ONMOUSEOVER_ARGUMENT];

      if (typeof method !== "function") {
        console.warn("cannot bind onmouseover: method target missing");
      }

      inputEl.addEventListener("mouseover", (ev) => {
        const detail = {value: UNKNOWN_VALUE};
        const invokeArgs = [detail, namespace, inputEl];
        if (arg !== undefined) invokeArgs.push(arg);
        method.apply(context, invokeArgs);
...
export function binder(root, app, binderOptions = null) {
  ...
  for (const [namespaceName, namespace] of Object.entries(state.namespaces)) {
    ...
    // For now popup does not have sub namespaces
    // therefore we expect there to be only one `popup` namespace
    // definition if any.
    if (namespaceName.startsWith(POPUP_PREFIX)) {
      // const popupName = namespaceName.substring(POPUP_PREFIX.length);
      const populEl = root.querySelector(`.${POPUP_PREFIX}`);
      if (!populEl) {
        // failed to find a popup element in the DOM
        continue;
      }
      binder.processPopup(populEl, namespace);
      populEl.addEventListener("submit", (ev) => ev.preventDefault());
    }
