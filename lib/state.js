...
# --- BEGIN_HEADER ---
#
# state.js - dual licensed source code file
...
function _wrapObservableArrayItemsIfPresent(
  ...
  for (const [arrayName, arrayDefinition] of Object.entries(arrayValues)) {
    ...
    if (isObservedArray(arrayDefinition)) {
      ...
      // and attached ot the namespace so use it
...
class State {
  ...
  get forms() {
    ...
    for (const namespaceName of Object.keys(this.namespaces)) {
      if (namespaceName.startsWith(_FORM_NAMESPACE_PREFIX)) {
  ...
  formState(formName) {
    return this.namespace(`${_FORM_NAMESPACE_PREFIX}${formName}`);
  }
  ...
  namespace(named) {
    if (!(named in this.namespaces)) {
      throw new Error("Tried to retrieve a namespace that does not exist");
    }
    return this.namespaces[named];
  }
  ...
  serializeNamespace(namedOrNamespace) {
    ...
    if (typeof namedOrNamespace === "string") {
      ...
      throw new Error(
        "Tried to serialize a namespace but was given a non-existing namespace or an incorrect type",
      );
    }
...
function _createState(definition, defaults, options) {
  ...
  for (const [named, allNamespaceItems] of Object.entries(namespaced)) {
    ...
    // Add valueNames to the serialization exclusion set
    // if they start with _ or is an HtmlObservable
    setAddAllIf(
...
export function createNamespacedState(
  ...
  // Define the forms namespace prefixes and state
